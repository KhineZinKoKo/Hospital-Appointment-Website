...
class Updateform1(forms.Form):
    ...
    mail = forms.EmailField(label='Email',
                            widget=forms.EmailInput(attrs={'type': 'email', 'placeholder': 'Enter your name'}))
    phno = forms.CharField(label='Phone no.', widget=forms.TextInput(attrs={'placeholder': 'Enter your phone number'}))
    dob = forms.DateField(label='Date of Birth',
                          widget=forms.DateInput(attrs={'placeholder': 'Enter your date of birth'}))
...
class PatientSignUpForm(forms.ModelForm):
    ...
    PatientPh = forms.CharField(
        max_length=11,
        label='Phone',
        validators=[RegexValidator(regex=r'^\d{1,11}$', message='Phone number must be up to 11 digits.')]
    )
