...
from .forms import UpdateForm, LoginForm, ScheduleForm, DateForm, DUpdateForm, searchForm, DoctorSignUpForm, \
    PatientSignUpForm
    ...
    if sid:
        ...
        if (from_date_query == '' or from_date_query is None) or (to_date_query == '' or to_date_query is None):
            if (patient_name_query != '' and patient_name_query is not None):
                ...
                message = "Search results for " + "'" + str(patient_name_query) + "'" + " : " + str(
                    appointment_table.count())
            if (doctor_name_query != '' and doctor_name_query is not None):
                ...
                message = "Search results for " + "'" + str(doctor_name_query) + "'" + " : " + str(
                    appointment_table.count())
            if (patient_name_query != '' and patient_name_query is not None) and (
                    doctor_name_query != '' and doctor_name_query is not None):
                appointment_table = Appointment.objects.filter(Patient_id__PatientName__icontains=patient_name_query,
                                                               Doctor_id__DoctorName__icontains=doctor_name_query)
                print("three ", appointment_table.query)
                message = "Search results for patient name " + "'" + str(
                    patient_name_query) + "'" + " and doctor name " + "'" + str(doctor_name_query) + "'" + " : " + str(
                    appointment_table.count())
        if (from_date_query != '' and from_date_query is not None) and (
                to_date_query != '' and to_date_query is not None):
            if ((patient_name_query != '' and patient_name_query is not None) and (
                    doctor_name_query != '' and doctor_name_query is not None)):
                appointment_table = Appointment.objects.filter(AppointDate__range=(from_date_query, to_date_query),
                                                               Patient_id__PatientName__icontains=patient_name_query,
                                                               Doctor_id__DoctorName__icontains=doctor_name_query)
                message = "Search results for patient name " + "'" + str(
                    patient_name_query) + "'" + ", Doctor name " + "'" + str(
                    doctor_name_query) + "'" + " and appointment dates between" + "'" + str(
                    from_date_query) + "'" + " and " + "'" + str(to_date_query) + "'" + " : " + str(
                    appointment_table.count())
                print("four ", appointment_table.query)
            elif (doctor_name_query != '' and doctor_name_query is not None):
                appointment_table = appointment_table.filter(AppointDate__range=(from_date_query, to_date_query),
                                                             Doctor_id__DoctorName__icontains=doctor_name_query)
                print("five ", appointment_table.query)
                message = "Search results for " + "'" + str(
                    doctor_name_query) + "'" + " and appointment dates between" + "'" + str(
                    from_date_query) + "'" + " and " + "'" + str(to_date_query) + "'" + " : " + str(
                    appointment_table.count())
                ...
                message = "Search results for " + "'" + str(
                    patient_name_query) + "'" + " and appointment dates between" + "'" + str(
                    from_date_query) + "'" + " and " + "'" + str(to_date_query) + "'" + " : " + str(
                    appointment_table.count())
            else:
                appointment_table = Appointment.objects.filter(AppointDate__range=(from_date_query, to_date_query))
                message = "Search results for appointment dates between" + "'" + str(
                    from_date_query) + "'" + " and " + "'" + str(to_date_query) + "'" + " : " + str(
                    appointment_table.count())
                print("seven ", appointment_table.query)
        return render(request, 'adminappoint.html',
                      {'items': s_data, 'appointment_table': appointment_table, 'message': message})
